...
bool NivelBase::HayColision(const sf::FloatRect& caja) {
...
void NivelBase::Actualizar(Juego &j) {
	...
	// Verifica si se cumplió la meta para pasar al siguiente nivel
	if (m_enemigosEliminados >= m_metaEnemigos) {
		int siguienteNivel = (m_idNivel + 1) % 4;  // Loop de 4 niveles
...
void NivelBase::ActualizarUI(float dt) {
	std::stringstream ss;
...
void NivelBase::ActualizarFisicas() {
...
void NivelBase::ProcesarCombate() {
	vector<BolaEnergia>* listaBolas = m_agil->ObtenerBolas();
	if (!listaBolas or listaBolas->empty()) return;
		...
		for (int j = m_enemigos.size() - 1; j >= 0; j--) {
			if (m_enemigos[j]->EstaVivo()) {
				if (cajaBola.intersects(m_enemigos[j]->ObtenerCaja())) {
					...
					if (m_enemigos[j]->EstaVivo() == false) {
						m_enemigosEliminados++;
					} 
...
void NivelBase::ProcesarEventos(Juego &j, Event &e) {
	if (m_juegoTerminado) {
		...
		if (!m_yaGuardo) {
			...
			if (e.key.code == Keyboard::R) j.PonerEscena(new Nivel1());
